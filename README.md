...
Although third-party cookies can enable sites to track user behavior across different top-level sites, there are also use cases on the web today where cross-domain subresources require some notion of session or persistent state that does not need to persist across different top-level sites.
